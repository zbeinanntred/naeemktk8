...
class TaskInProgress {
  ...
  synchronized boolean updateStatus(TaskStatus status) {
    ...
    if (oldStatus != null) {
        //for the incremental iterative one-job case
    	LOG.info("old status iteration " + oldStatus.getIteration() + " new status iteration " + status.getIteration());
        if(oldStatus.getIteration() != status.getIteration()) return true;
