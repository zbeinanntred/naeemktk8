...
class ReduceTask extends Task {
  ...
  static class SourceValuesIterator<KEY,VALUE,SOURCEKEY> implements Iterator<VALUE> {
	    ...
	    private IFile.TrippleWriter<KEY, VALUE, SOURCEKEY> writer;
	    ...
	    public SourceValuesIterator (RawKeyValueSourceIterator in, 
	      ...
	      Path preservePath = new Path(job.getPreserveStatePath() + "/reducePreserve-" + taskid);
	      FileSystem hdfs = FileSystem.get(job);
	      FSDataOutputStream preserveOut = hdfs.create(preservePath);
	      writer = new IFile.TrippleWriter<KEY, VALUE, SOURCEKEY>(job, preserveOut, 
	    ...
	    private void readNextSource() throws IOException {
	      ...
	      skeyIn.reset(nextSourceBytes.getData(), nextSourceBytes.getPosition(), nextSourceBytes.getLength());
	...
	/**
	 * for extracting the useful k [v] for reduce function, reading from the delta file and the preserve file
	 * @author yzhang
	 *
	 * @param <KEY>
	 * @param <VALUE>
...
//				System.out.println("*3 C:" + Arrays.toString(lineC));
	  ...
	  class PreserveFileReader{
		  ...
		  public PreserveFileReader(RawKeyValueSourceIterator in, JobConf job,
                  ...
                  Class<VALUE> valClass, 
                  Class<SOURCEKEY> skeyClass,
                  RawComparator<KEY> com1) throws IOException{
			  this.in = in;
		      SerializationFactory serializationFactory = new SerializationFactory(job);
		      this.keyDeserializer = serializationFactory.getDeserializer(keyClass);
		      this.keyDeserializer.open(keyIn);
		  ...
		  public KSVTuple getNextValue(KEY key) throws IOException{
			  //LOG.info("get next value for key " + key + " bnext " + bNext);
			  if (bNext) {
				  //if we are processing the new key, and it happens to be the next line of the preserve file
				  
				  //for single preserve file
				  if (record == null){
					  bNext = false;
				  } else if (record.key.equals(key)){
					  KSVTuple res = new KSVTuple(record.key, record.source, record.value);
					  //cachedSource = res.source;
					  record = null;
					  bNext = false;
					  return res;
				  } else{
			  ...
			  if(record == null){
				  //LOG.info("reading preserve file");
				  while (in.next()){
					  
					  KEY pkey = readKey();

					  //these may impact performance, can I read it only when it is useful?
			  ...
			  //LOG.info("preserve record match is " + record);
			  
			  if (record == null) return null;		//pass all entries
			  
			  //same key or next key
			  if(record.key.equals(key)){
				  KSVTuple res = new KSVTuple(record.key, record.source, record.value);
				  record = null;
				  return res;
		  ...
		  //set bNext to be true, indicating that we are moving to the next key
		  public void setNextKey(){
			  bNext = true;
		  }
		  
		  public void writeRest() throws IOException{
			  if (record != null) pwriter.append(record.key, record.value, record.source);
	    ...
	    private PreserveFileReader preserveReader;
	    ...
	    public IncrementalSourceValuesIterator (RawKeyValueSourceIterator in, 
	      ...
	      if(currPreserveRecord == null) throw new RuntimeException("no entries in preserve file!!!");
  ...
  private class OldTrackingRecordWriter<K, V> implements RecordWriter<K, V> {
    ...
    public OldTrackingRecordWriter(
        ...
        JobConf job, TaskReporter reporter, String finalName, boolean onhdfs)
        throws IOException {
      ...
      if(onhdfs){
    	  ...
    	  /*
		FileSystem localfs = FileSystem.getLocal(job);
		FSDataOutputStream fileOut = localfs.create(new Path(finalName), reporter);
  ...
  private class OldTrackingIFileRecordWriter<K, V> implements RecordWriter<K, V> {
  ...
  private class WrappedOutputLocalCollector<INKEY,INVALUE,OUTKEY,OUTVALUE> implements OutputCollector<OUTKEY,OUTVALUE>{
  ...
  private class FilterOutputLocalCollector<INKEY,INVALUE,OUTKEY,OUTVALUE extends Writable> implements OutputCollector<OUTKEY,OUTVALUE>{
	  private RecordWriter<OUTKEY,OUTVALUE> localwriter;
	  ...
	  public FilterOutputLocalCollector(RecordWriter<OUTKEY,OUTVALUE> localwriter, 
			  RecordWriter<OUTKEY,OUTVALUE> filterwriter, TaskReporter inreporter, 
			  ResultFileQueue<INKEY,INVALUE,OUTKEY,OUTVALUE> inResultQueue) throws IOException{
		  this.localwriter = localwriter;
		  ...
		  reducer = ReflectionUtils.newInstance(conf.getIterativeReducerClass(), conf);
		  
		  resultQueue = inResultQueue;
		  more = resultQueue.next();
		  if(!more) throw new RuntimeException("no more data to read!!!");
	...
	public void collect(OUTKEY key, OUTVALUE value) throws IOException {
		...
		while(more && comparator.compare(lastResultKey, key) < 0){
			...
			if(bwrite) localwriter.write(lastResultKey, resultQueue.getValue());
  ...
  private class WrappedOutputBothCollector<INKEY,INVALUE,OUTKEY,OUTVALUE> implements OutputCollector<OUTKEY,OUTVALUE>{
	  private RecordWriter<OUTKEY,OUTVALUE> localwriter;
	  ...
	  public WrappedOutputBothCollector(RecordWriter<OUTKEY,OUTVALUE> localwriter, 
			  RecordWriter<OUTKEY,OUTVALUE> hdfswriter, TaskReporter inreporter, 
			  TerminateChecker<INKEY,INVALUE,OUTKEY,OUTVALUE> intermchecker) throws IOException{
		  this.localwriter = localwriter;
  ...
  private class TerminateChecker<INKEY,INVALUE,OUTKEY,OUTVALUE> {
	  ...
	  public TerminateChecker(int currIteration, Reporter reporter) throws IOException{
	  ...
	  public float accumulateDistance(OUTKEY key, OUTVALUE oldvalue, OUTVALUE newvalue) throws IOException{
		  float difference = reducer.distance(key, oldvalue, newvalue);
		  distance += difference;
		  return difference;
	  }
	  
	  public float getSingleDistance(OUTKEY key, OUTVALUE oldvalue, OUTVALUE newvalue) throws IOException{
  ...
  private class ResultFileQueue<INKEY,INVALUE,OUTKEY,OUTVALUE> 
  	extends PriorityQueue<ResultFile<OUTKEY,OUTVALUE>> {
	  
	  //List<ResultFile<OUTKEY,OUTVALUE>> rfile = new ArrayList<ResultFile<OUTKEY,OUTVALUE>>();
	  ...
	  public ResultFileQueue(int currIteration, Reporter reporter) throws IOException{
		  taskid = getTaskID().getTaskID().getId();
		  comparator = conf.getOutputKeyComparator();
		  
		  /*
		  //for multiple preserve files
		  if(conf.isIncrementalStart()){
			  initialize(1);
		  }else if(conf.isIncrementalIterative()){
	  ...
	  private void adjustPriorityQueue(ResultFile<OUTKEY,OUTVALUE> reader) throws IOException{
      ...
      //for multiple preserve file
	  private void addResultFiles2(int currentIteration, Reporter reporter) throws IOException{
		  /*
		  rfile.add(new ResultFile(getOldPrevResultReader(conf, reporter), 0));
		  for(int i=1; i<currentIteration; i++){
			  rfile.add(new ResultFile(getIncrementalPrevResultReader(conf, i, reporter), i));
		  }
		  */
		  put(new ResultFile(getOldPrevResultReader(conf, reporter), 0));
		  for(int i=1; i<=currentIteration; i++){
			  put(new ResultFile(getIncrementalPrevResultReader(conf, i, reporter), i));
		  }
	  }
	  
	  //for single preserve file
	  private void addResultFiles(int currentIteration, Reporter reporter) throws IOException{
		  /*
		  rfile.add(new ResultFile(getOldPrevResultReader(conf, reporter), 0));
		  for(int i=1; i<currentIteration; i++){
			  rfile.add(new ResultFile(getIncrementalPrevResultReader(conf, i, reporter), i));
		  ...
		  */
		  if(currentIteration == -1){
			  ...
			  put(new ResultFile(getIncrementalPrevResultReader(conf, (currentIteration-1), reporter), 0));
		  }
	  }

	  	private RecordReader<OUTKEY,OUTVALUE> getOldPrevResultReader(JobConf job, Reporter reporter) throws IOException{
			if(job.getDynamicDataPath() == null) throw new IOException("we need the converged result data " +
					"to perform incremental computation!!!");
			
			//download the remote old static data
			Path remotePrevConvPath = new Path(job.getDynamicDataPath() + "/" + getOutputName(taskid));
			Path localPrevConvPath = new Path("/tmp/iteroop/" + job.getIterativeAlgorithmID() + "/prevconv-" + taskid);
			...
			if(hdfs.exists(remotePrevConvPath)){
				...
				if(!localfs.exists(localPrevConvPath)){
					...
					LOG.info("copy remote old converged file " + remotePrevConvPath + " to local disk" + localPrevConvPath + "!!!!!!!!!");
				}
				
				//load static data
				long filelen = localfs.getFileStatus(localPrevConvPath).getLen();
				InputSplit inputSplit = new FileSplit(localPrevConvPath, 0, filelen, null, true);
				return conf.getResultInputFormat().getRecordReader(inputSplit, conf, reporter);
			}else{
				throw new IOException("acturally, there is no previous converged result on the path you" +
					" have set! Please check the path and check the map task number " + remotePrevConvPath);
			}
		}
  	
	  	private RecordReader<OUTKEY,OUTVALUE> getIncrementalPrevResultReader(JobConf job, int iteration, Reporter reporter) throws IOException{
	
			//download the remote old static data
			Path remotePrevConvPath = new Path(job.getDynamicDataPath() + "/" + getOutputName(getPartition()));
			Path localPrevConvPath = new Path("/tmp/iteroop/" + job.getIterativeAlgorithmID() + "/substate-" + iteration + "." + taskid);
			
			if(!localfs.exists(localPrevConvPath)){
				if(hdfs.exists(remotePrevConvPath)){
					hdfs.copyToLocalFile(remotePrevConvPath, localPrevConvPath);
					LOG.info("copy remote incremental previous result file " + remotePrevConvPath + " to local disk" + localPrevConvPath + "!!!!!!!!!");
				}else{
					throw new IOException("acturally, there is no previous converged result on the path you" +
							" have set! Please check the path and check the map task number " + remotePrevConvPath);
				}
  ...
  void runPreserveReducer(JobConf job,
                     TaskUmbilicalProtocol umbilical,
                     final TaskReporter reporter,
                     RawKeyValueSourceIterator rIter,
                     RawComparator<INKEY> comparator,
                     Class<INKEY> keyClass,
  ...
  private <OUTKEY extends Writable,OUTVAL extends GlobalRecordable> void sendGlobalData(JobConf job, String localResult, TaskReporter reporter) throws IOException{
	  ...
	  OUTKEY key = resultReader.createKey();
	  OUTVAL value = resultReader.createValue();
	  ...
	  while(resultReader.next(key, value)){
		  ...
		  //record.readObject(value.toString());
		  gvalue.put(key, value);
		  key = resultReader.createKey();
  ...
  void runIncrementalReducer(JobConf job,
                     ...
                     final TaskReporter reporter,
                     RawKeyValueSourceIterator rIter,
                     RawKeyValueSourceIterator pIter,
                     RawComparator<INKEY> comparator,
                     Class<INKEY> keyClass,
                     Class<INVALUE> valueClass,
                     Class<SOURCEKEY> skeyClass) throws IOException {
  ...
  class ReduceCopier<K, V, SK> implements MRConstants {
    ...
    @SuppressWarnings("unchecked")
    private RawKeyValueSourceIterator createKVSIterator(
        JobConf job, FileSystem fs, Reporter reporter) throws IOException {
      ...
      if (mapOutputsFilesInMemory.size() > 0) {
        ...
        if (numMemDiskSegments > 0 &&
          ...
          final Path outputPath =
              mapOutputFile.getInputFileForWrite(mapId, inMemToDiskBytes);
          final RawKeyValueSourceIterator rIter = PreserveMerger.merge(job, fs,
              keyClass, valueClass, skeyClass, memDiskSegments, numMemDiskSegments,
              tmpDir, comparator, comparator2, reporter, spilledRecordsCounter, null);
          final TrippleWriter writer = new TrippleWriter(job, fs, outputPath,
              keyClass, valueClass, skeyClass, codec, null);
          try {
      ...
      for (Path file : onDisk) {
        onDiskBytes += fs.getFileStatus(file).getLen();
        diskSegments.add(new KVSSegment<K,V,SK>(job, fs, file, codec, keepInputs, -1));
      }

      LOG.info("Merging " + onDisk.length + " files, " +
               onDiskBytes + " bytes from disk");
      Collections.sort(diskSegments, new Comparator<KVSSegment<K,V,SK>>() {
		@Override
      ...
      if (0 != onDiskBytes) {
        ...
        diskSegments.addAll(0, memDiskSegments);
        memDiskSegments.clear();
        RawKeyValueSourceIterator diskMerge = PreserveMerger.merge(
            job, fs, keyClass, valueClass, skeyClass, codec, diskSegments,
            ioSortFactor, numInMemSegments, tmpDir, comparator, comparator2,
            reporter, false, spilledRecordsCounter, null);
        diskSegments.clear();
        if (0 == finalSegments.size()) {
          return diskMerge;
        ...
        finalSegments.add(new KVSSegment<K,V,SK>(
              new RawKVSIteratorReader(diskMerge, onDiskBytes), true, -1));
      }
      return PreserveMerger.merge(job, fs, keyClass, valueClass, skeyClass,
                   finalSegments, finalSegments.size(), tmpDir,
    ...
    private RawKeyValueSourceIterator createPreserveKVSIterator2(
      ...
      return PreserveMerger.merge(job, fs, keyClass, valueClass, skeyClass,
    		  preserveSegments, preserveSegments.size(), tmpDir,
                   comparator, comparator2, reporter, spilledRecordsCounter, null);
    }
    ...
    class RawKVSIteratorReader extends IFile.TrippleReader<K,V,SK> {
	    ...
	    public RawKVSIteratorReader(RawKeyValueSourceIterator kvsIter, long size)
	        throws IOException {
	      super(null, null, size, null, spilledRecordsCounter);
	      this.kvsIter = kvsIter;
	    }
	
	    public boolean next(DataInputBuffer key, DataInputBuffer value, DataInputBuffer skey)
	        throws IOException {
	      if (kvsIter.next()) {
      ...
      private int getMapCompletionEvents() throws IOException {
        ...
        for (TaskCompletionEvent event : events) {
          switch (event.getTaskStatus()) {
            case SUCCEEDED:
            {
            	LOG.debug("SUCCEEDED event is received!");
            ...
            case RUNNING:
            {
            	LOG.debug("RUNNING event is received!");
              URI u = URI.create(event.getTaskTrackerHttp());
              String host = u.getHost();
              TaskAttemptID taskId = event.getTaskAttemptId();
              URL mapOutputLocation = new URL(event.getTaskTrackerHttp() + 
