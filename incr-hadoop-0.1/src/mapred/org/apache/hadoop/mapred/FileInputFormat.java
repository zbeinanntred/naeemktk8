...
public abstract class FileInputFormat<K, V> implements InputFormat<K, V> {
   ...
   */
  /**
   * if it is iterative related jobs, since we already partition the data based on some joinable property, we don't want
   * to impact our partition strategy, so we don't allow repartition the data partition
   */
  protected boolean isSplitable(JobConf job, FileSystem fs, Path filename) {
	  if(job.isIterative() || job.isIncrementalStart() || job.isIncrementalIterative() || job.isPreserve()){
		  return false;
	  }
  ...
  public InputSplit[] getSplits(JobConf job, int numSplits)
    ...
    for (FileStatus file: files) {
      ...
      if ((length != 0) && isSplitable(job, fs, path)) { 
