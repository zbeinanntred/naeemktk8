...
public class JobInProgress {
  ...
  //iterative split to map task map
  public Map<Integer, Integer> splitTaskMap = new HashMap<Integer, Integer>();
  ...
  throws IOException, KillInterruptedException, UnknownHostException {
    ...
    for(int i=0; i < numMapTasks; ++i) {
      ...
      //for iterative job
      splitTaskMap.put(splits[i].getSplitIndex().getSplitIndex(), i);
    }
  ...
  public synchronized void updateTaskStatus(TaskInProgress tip, 
    ...
    if (change) {
      ...
      if (state == TaskStatus.State.SUCCEEDED) {
        ...
        tip.setSuccessEventNumber(taskCompletionEventTracker); 
      } else if (state == TaskStatus.State.RUNNING) {
    	  ...
    	  LOG.info("creating a new task event! " + taskEvent);
      ...
      if (taskEvent != null) {
    	  LOG.info("task completion events added! " + taskEvent + " event size: " + taskCompletionEvents.size());
  ...
  public synchronized Task obtainNewNodeOrRackLocalMapTask(
	      TaskTrackerStatus tts, int clusterSize, int numUniqueHosts, int target)
	  throws IOException {
	    if (!tasksInited) {
	      LOG.info("Cannot create task split for " + profile.getJobID());
	      try { throw new IOException("state = " + status.getRunState()); }
