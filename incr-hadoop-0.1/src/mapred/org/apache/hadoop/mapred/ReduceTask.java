...
//				System.out.println("*3 C:" + Arrays.toString(lineC));
  ...
  class ReduceCopier<K, V, SK> implements MRConstants {
    ...
    private RawKeyValueSourceIterator createPreserveKVSIterator(
      ...
      if(job.isIncrementalStart()){
	      int taskid = reduceTask.getTaskID().getTaskID().getId();
	      Path remotePreservedStatePath = new Path(job.getPreserveStatePath() + "/preserve-" + taskid);
	      Path localPreservedStatePath = new Path("/tmp/iteroop/" + job.getIterativeAlgorithmID() + "/preserve-" + taskid);
	      ...
	      Path remoteIndexPreservedStatePath = new Path(job.getPreserveStatePath() + "/preserve-" + taskid + ".index");
	      Path localIndexPreservedStatePath = new Path("/tmp/iteroop/" + job.getIterativeAlgorithmID() + "/preserve-" + taskid + ".index");
	  			
      	  if(hdfs.exists(remoteIndexPreservedStatePath)){
      		  //if it doesn't exist the local static file, it means it is the first iteration, so copy it from hdfs
      		  if(!localfs.exists(localIndexPreservedStatePath)){
      			  hdfs.copyToLocalFile(remoteIndexPreservedStatePath, localIndexPreservedStatePath);
      			  LOG.info("copy remote preserve file " + remoteIndexPreservedStatePath + " to local disk" + localIndexPreservedStatePath + "!!!!!!!!! and file size is " + localfs.getFileStatus(localIndexPreservedStatePath).getLen());
      		  }
  				
      		  //use RandomAccessKSVFile
      		  //adsfsd.add(new KVSSegment<K,V,SK>(job, fs, localIndexPreservedStatePath, null, true, 0));
      		  
      	  }else{
      		  throw new IOException("acturally, there is no preserve data on the path you" +
  					" have set! Please check the path and check the map task number " + remoteIndexPreservedStatePath);
      	  }
      }else if(job.isIncrementalIterative()){
    	  //the preserve file stored in last snapshot
      	  taskid = reduceTask.getTaskID().getTaskID().getId();
